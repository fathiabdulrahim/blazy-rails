...
task :submodule do
  sh 'git submodule update --init' unless File.exist?('Blazy/README.md')
end
...
task javascripts: :submodule do
  ...
  FileUtils.cp_r Dir['Blazy/compiled/*.js'], target_dir
end
...
task stylesheets: :submodule do
  ...
  FileUtils.cp Dir['Blazy/compiled/*.css'], target_dir
end
